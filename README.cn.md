...
clarification ─── 解决歧义，探索代码库
    ...
    |       |── 复杂（评分 10-15）
    |               |
    |           milestone-planning ─── 探索 → 草案 → 对抗性评审
    |               |
    |           long-run ─── 多日编排器
...
先探索代码库，再基于探索发现进行迭代式问答，将模糊请求收窄为明确的工作范围。输出带有自动复杂度路由的 Context Brief。
...
通过广泛的并行探索构建草案里程碑 DAG，然后提交给独立的对抗性评审者——隐藏复杂度、依赖与顺序、可验证性、集成风险——评审者逐轮攻击草案，直到不再有阻断性（Blocking）发现。

**触发条件：** "规划里程碑"、"分解为里程碑"、"ultraplan"，或 clarification 给出复杂（Complex）判定（评分 10-15）后。

**主要特性：**
- 两套草案骨架（接口优先 vs 风险优先）以对抗锚定效应
- 信息隔离的独立对抗性评审者（无交叉污染）
- 带逐字（verbatim）解决日志的 critique-until-dry 修订循环
- 每个里程碑强制二元可判定（binary-decidable）成功标准
- 里程碑数量守卫（超过 7 个警告，超过 10 个需要批准）

#### Long Run Harness（长期运行治具）

编排多天执行。每个里程碑经过 plan-crafting、run-plan、review-work，支持检查点/恢复。自主运行——里程碑锁定是唯一的批准关口，仅在失败时上报。
